...
- Container images are stored in **AWS ECR** (provisioned by Terraform)
...
| `cloudwatch` | CloudWatch log group `/ecs/<app_name>` |
...
The pipeline is defined in `.github/workflows/deploy.yml` and triggers automatically on every push to `main`. It is structured as four separate jobs to isolate failures clearly.
...
**Trivy security scanning** — every Docker image is scanned for `HIGH` and `CRITICAL` CVEs before being pushed. `ignore-unfixed: true` filters out vulnerabilities with no available fix (common in base images). `exit-code: 0` keeps the pipeline non-blocking while still surfacing the report in the Actions log.

**Deploy job disabled (`if: false`)** — the deploy job is fully written and configured but intentionally gated. The AWS account is not yet active for this challenge. Enabling it requires only removing the `if: false` condition and populating the AWS secrets — no other changes needed.
...
Terraform provisions an ECR repository as part of the infrastructure definition. During this challenge the CI pipeline pushes to Docker Hub because the AWS account is not yet active. The switch to ECR in production requires only updating the image URL in the pipeline — the repository is already provisioned.
